...
import cv2 # This one is already there
...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    def __init__(self, *,  # only keyword arguments
                 ...
                 n_tentative_crops=4,
                 seed=None,
                 # --- Lunar Augmentations Parameters ---
                 use_lunar_augmentations=False,
                 lunar_grayscale_tint_prob=0.3, # Probability to apply grayscale tint
                 lunar_grayscale_alpha_range=(0.3, 1.0),
                 lunar_bilateral_filter_prob=0.5, # Probability to apply bilateral filter
                 lunar_bilateral_params=(15, 75, 100),
                 lunar_contrast_variation_prob=0.7, # Probability to apply contrast variation
                 lunar_contrast_factors=[0.5, 1.0, 1.5, 2.0],
                 # --- End Lunar Augmentations Parameters ---
                 ):
        ...
        # --- Initialize Lunar Augmenter ---
        self.use_lunar_augmentations = use_lunar_augmentations
        if self.use_lunar_augmentations:
            self.lunar_grayscale_tint_prob = lunar_grayscale_tint_prob
            self.lunar_bilateral_filter_prob = lunar_bilateral_filter_prob
            self.lunar_contrast_variation_prob = lunar_contrast_variation_prob

            # The LunarAugmenter instance will be created once
            self.lunar_augmenter = self._LunarAugmenter(
                alpha_range=lunar_grayscale_alpha_range,
                contrast_factors=lunar_contrast_factors,
                bilateral_params=lunar_bilateral_params
            )
        else:
            self.lunar_augmenter = None
        # --- End Initialize Lunar Augmenter ---


    # --- Define LunarAugmenter as an inner class for self-containment ---
    # This keeps Mast3r code organized and prevents polluting the global namespace.
    class _LunarAugmenter:
        def __init__(self, alpha_range=(0.3, 1.0), contrast_factors=[0.5, 1.0, 1.5, 2.0],
                     bilateral_params=(15, 75, 100)):
            self.alpha_range = alpha_range
            self.contrast_factors = contrast_factors
            self.bilateral_d, self.bilateral_sigmaColor, self.bilateral_sigmaSpace = bilateral_params

        def apply_grayscale_tint(self, image_gray, rng):
            """Random grayscale tint variation, using the dataset's RNG."""
            alpha = rng.uniform(*self.alpha_range)
            modified = np.clip(image_gray.astype(np.float32) * alpha, 0, 255).astype(np.uint8)
            return modified

        def apply_bilateral_filter(self, image_bgr):
            """Apply bilateral filter."""
            filtered = cv2.bilateralFilter(image_bgr, d=self.bilateral_d,
                                           sigmaColor=self.bilateral_sigmaColor,
                                           sigmaSpace=self.bilateral_sigmaSpace)
            return filtered

        def get_random_contrast_factor(self, rng):
            """Get a random contrast factor."""
            return rng.choice(self.contrast_factors)
    # --- End LunarAugmenter inner class ---
    ...
    def __getitem__(self, idx):
        ...
        for v, view in enumerate(views):
            ...
            # --- Apply Lunar Augmentations ---
            if self.use_lunar_augmentations and self.lunar_augmenter:
                # Convert PIL Image to OpenCV format (numpy array BGR) for bilateral filter
                # PIL is RGB, OpenCV is BGR
                img_np = np.array(view['img'].convert('RGB'))
                img_bgr = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)

                # Apply Bilateral Filter
                if self._rng.random() < self.lunar_bilateral_filter_prob:
                    img_bgr = self.lunar_augmenter.apply_bilateral_filter(img_bgr)

                # Convert back to PIL Image for tint and contrast
                img_pil = Image.fromarray(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB))

                # Apply Grayscale Tint
                if self._rng.random() < self.lunar_grayscale_tint_prob:
                    img_gray = np.array(img_pil.convert('L'))
                    modified_gray = self.lunar_augmenter.apply_grayscale_tint(img_gray, self._rng)
                    img_pil = Image.fromarray(modified_gray).convert('RGB')

                # Apply Contrast Variation
                if self._rng.random() < self.lunar_contrast_variation_prob:
                    random_factor = self.lunar_augmenter.get_random_contrast_factor(self._rng)
                    enhancer = ImageEnhance.Contrast(img_pil)
                    img_pil = enhancer.enhance(random_factor)

                view['img'] = img_pil # Update the image with the augmented PIL image
            # --- End Lunar Augmentations ---

            view['img'] = self.transform(view['img']) # Apply ImgNorm or ColorJitter
