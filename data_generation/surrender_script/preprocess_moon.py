...
def read_meta(npz_path: Path):
    ...
    if not all(k in data for k in required):
        ...
        raise KeyError(f"{npz_path.name}:  {missing}")
...
def process_one(img_path: Path, npz_path: Path, out_dir: Path):
    ...
    if img_bgr is None:
        raise ValueError("impossible to read the image")
...
def main(images_dir, metadata_dir, output_dir):
    ...
    for img_path in tqdm(images):
        if img_path.suffix.lower() not in VALID_IMG_EXT:
            continue
        npz_path = meta_dir / img_path.with_suffix(".npz").name  
        print(npz_path)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--images_dir",   required=True, help="image folder (.png, .jpg …)")
    ap.add_argument("--metadata_dir", required=True, help="metadata folder (sous-dossiers par image)")
