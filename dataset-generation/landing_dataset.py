...
using the SurRender rendering engine. Unlike standard nadir or simple oblique
imaging, this creates varied and challenging viewing geometries.

DYNAMIC CAMERA MODES:
---------------------
Three camera modes create increasingly varied stereo configurations:

1. "stable" (30% probability):
   - Pitch variation: ±5° from target direction
   - Both cameras maintain similar orientations
   - Classical stereo geometry with minor variations
   - Best for baseline stereo reconstruction

2. "moderate" (40% probability):
   - Pitch variation: ±10° from target direction  
   - Cameras can have moderately different orientations
   - Tests stereo matching with geometric variation
   - Good balance of challenge and feasibility

3. "dynamic" (30% probability):
   - Pitch variation: ±15° from target direction
   - Cameras may have significantly different viewing angles
   - Creates challenging stereo pairs for robust algorithms
   - Maximum geometric diversity

KEY FEATURES:
-------------
- Both cameras in a pair can have DIFFERENT viewing modes
- Each camera independently varies altitude (±30%)
- Viewing constraints allow up to ±30° deviation from target
- Roll variations add additional geometric complexity
- B/H ratio maintains consistent stereo geometry across altitudes

VIEWING CONSTRAINT LEVELS:
--------------------------
- "tight": ±10° max deviation (easier stereo matching)
- "moderate": ±20° max deviation (standard challenge)
- "loose": ±30° max deviation (maximum challenge)
...
# ════════════════════════════════════════════════════════════════════════════════
# Creates varied stereo geometries where both cameras can have
# different configurations for diverse and challenging stereo pairs.

CAMERA_MODES = ["stable", "moderate", "dynamic"]

# Pitch adjustment for each mode (added on top of target-pointing direction)
STABLE_PITCH_RANGE = (-5, 5)      # Minor variations, classical stereo
MODERATE_PITCH_RANGE = (-10, 10)  # Medium variations
DYNAMIC_PITCH_RANGE = (-15, 15)   # Large variations, challenging stereo
...
def check_infinity_in_view(s, position, attitude):
    ...
    Important for dynamic stereo pairs where varied viewing angles
...
def generate_camera_config(P_surf, alt_ground, base_altitude, camera_id, target_point=None, constraint_type="moderate"):
    """
    Generate a DYNAMIC camera configuration for varied stereo pairs.
    
    DYNAMIC STEREO CAMERA CONFIGURATION:
    ------------------------------------
    Unlike nadir/oblique scripts where cameras maintain consistent geometry,
    this function creates VARIED configurations for challenging stereo pairs:
    
    - Each camera INDEPENDENTLY chooses its viewing mode
    - Altitude varies ±30% from base (cameras at different heights)
    - Pointing can deviate up to ±30° from target (constraint dependent)
    - Roll adds rotational complexity
    
    This creates diverse stereo pairs where the two images may have:
    ...
    constraint_type : str
        Stereo difficulty level:
        - "tight": ±10° max deviation (easier matching)
        - "moderate": ±20° max deviation (standard)
        - "loose": ±30° max deviation (extreme challenge)
    ...
    # Choose a random viewing mode
    # 30% stable, 40% maneuvering, 30% aggressive
    ...
    # Apply additional rotations based on mode
    if mode == "stable":
        # Stable: minor variations, classical stereo geometry
        pitch_adjustment = np.random.uniform(-5, 5)
    elif mode == "moderate":
        # Moderate: medium variations for diversity
        pitch_adjustment = np.random.uniform(-10, 10)
    else:  # dynamic
        # Dynamic: large variations for challenging stereo pairs
...
if __name__ == "__main__":
    ...
    for alt_level, alt_config in enumerate(ALTITUDE_DISTRIBUTION):
        ...
        altitude_stats = {
            "modes": {"stable": 0, "moderate": 0, "dynamic": 0},
            "lat_coverage": {},
            "pitch_ranges": [],
            "roll_ranges": [],
            "baselines": [],
    ...
    df_cam.to_csv("output/camera_trajectory_dynamic.csv", index=False)
    ...
    df_sun.to_csv("output/sun_trajectory_dynamic.csv", index=False)
    
    # Detailed metadata
    df_meta = pd.DataFrame(metadata)
    df_meta.to_csv("output/metadata_dynamic.csv", index=False)
    ...
    df_summary.to_csv("output/pair_summary_dynamic.csv", index=False)

    # ════════════════════════════════════════════════════════════════════════════════
    # FINAL REPORT
    # ════════════════════════════════════════════════════════════════════════════════
    print("\n" + "="*80)
    print("FINAL GENERATION REPORT (DYNAMIC STEREO)")
    print("="*80)
    
    print(f"\n✅ Files exported:")
    print("   • output/camera_trajectory_dynamic.csv")
    print("   • output/sun_trajectory_dynamic.csv")
    print("   • output/metadata_dynamic.csv")
    print("   • output/pair_summary_dynamic.csv")
    ...
    print("- Modes: nadir, oblique, strong_oblique")
